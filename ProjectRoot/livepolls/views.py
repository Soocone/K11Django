'''
단축함수: Django에서 웹 프로그램 개발시 자주 사용하는 기능들을
...
# 투표 처리: 선택한 항목의 vote 컬럼을 +1 해준다.
def vote(request, question_id):
    # 선택한 질문의 일련번호를 통해 레코드를 가져온다.
    question = get_object_or_404(Question, pk=question_id)
    try:
        # detail 페이지에서 선택한 항목을 통해 choice 테이블에서 레코드를 가져온다.
        ...
        # try 절의 코드가 정상실행되면 가져온 레코드를 통해 votes 항목을 +1 업데이트 한다.
